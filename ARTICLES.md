# Article plan – lexpraktyk.pl

Priority: high-volume search queries with real legal pain behind them, in the three site categories
(Nieruchomości / Biznes / Reputacja). Titles keyword-first, matching real search phrasing.

Style rules: see README (natural language, blockquote for the key warning, Podstawa prawna with Dz.U. refs).
**Length scales with query competitiveness** — niche 750–1000 words, contested 1200+ (README § Styl artykułów).
Publishing: future `publishDate` + daily 05:00 UTC rebuild = auto-publish; preview at `/preview/[slug]`.

## Prioritisation basis (Search Console, 27 May – 18 Aug 2026)

269 impressions, 1 click, average position 66. Small sample — treat as direction, not proof.
Re-check quarterly and re-tier accordingly.

**Where we actually rank (top 20):** medical/reputation niche — `opinia-od-pacjenta` poz. 7,7 ·
`umowa-przedwstepna` poz. 7,0 · `ochrona-wizerunku-lekarza` poz. 15,2. Low competition, low volume,
but we win there.

**Where we have demand but no ranking (poz. 54–100):** eksmisja/najem cluster (~60 impr. across 15
query variants) · windykacja + nieuczciwy kontrahent B2B (~36 impr.) · zachowek cluster (~30 impr.).
These need 1200+ word depth, not more thin articles.

**Where we have neither:** the transactional Nieruchomości topics (koszty notarialne, zadatek,
cesja, licytacja…). Zero impressions in 3 months. Not disproven — just unevidenced, so they wait.

Consequence: the old 20/11/7 split (Nieruchomości/Biznes/Reputacja) was close to backwards.
Retiered below.

## Published (27)

- [x] Testament: jak napisać żeby był ważny
- [x] Odrzucenie spadku: kiedy warto i jak to zrobić w 6 tygodniach
- [x] Stwierdzenie nabycia spadku: sąd czy notariusz
- [x] Dział spadku: jak podzielić majątek po śmierci bliskiej osoby
- [x] Podatek od spadku: zwolnienia i terminy
- [x] Zachowek: komu przysługuje i jak go obliczyć
- [x] Zakup mieszkania bez agenta
- [x] Spór z deweloperem: wady mieszkania
- [x] Wypowiedzenie umowy o pracę: prawa pracownika
- [x] Zakaz konkurencji: kiedy jest ważny
- [x] Pułapki w umowach B2B: klauzule abuzywne
- [x] Pomówienie w internecie: co zrobić
- [x] Fałszywe opinie na Facebooku i forach
- [x] RODO a opinie w internecie
- [x] Fałszywa opinia na Google: jak ją usunąć
- [x] Szantaż reputacyjny: klient grozi 1★
- [x] Opinia od pacjenta którego nigdy nie leczyłeś
- [x] Anonimowe pomówienie — ustalenie tożsamości
- [x] Ochrona wizerunku firmy w internecie
- [x] Umowa przedwstępna: 7 pułapek
- [x] Odbiór mieszkania od dewelopera: checklista
- [x] Księga wieczysta: jak czytać
- [x] Zakup mieszkania na rynku wtórnym
- [x] Deweloper opóźnia oddanie mieszkania
- [x] Jak zabezpieczyć się przed nieuczciwym kontrahentem B2B
- [x] Umowa z wykonawcą: co wpisać
- [x] Ochrona wizerunku lekarza w internecie

## Scheduled — batch 2 (10 — every 5 days, Sep 4 – Oct 19, 2026)

Pierwsza partia napisana już pod nową regułę długości (977–1321 słów).
Klastry przeplatane celowo, żeby nie publikować sześciu tekstów reputacyjnych z rzędu.

- [x] 2026-09-04 · Tajemnica zawodowa a negatywna opinia: dlaczego nie możesz się bronić publicznie *(flagowy, 1321 sł.)*
- [x] 2026-09-09 · Jak odpowiedzieć na negatywną opinię, nie łamiąc tajemnicy zawodowej *(szablony)*
- [x] 2026-09-14 · Klient żąda zwrotu pieniędzy po wykonanej usłudze: kiedy musisz oddać
- [x] 2026-09-19 · Wydziedziczenie a zachowek: kiedy pozbawienie zachowku jest skuteczne
- [x] 2026-09-24 · Groźba negatywnej opinii jako wymuszenie: kiedy to przestępstwo
- [x] 2026-09-29 · Zachowek od darowizny: gdy spadkodawca rozdał majątek za życia
- [x] 2026-10-04 · Umowa i protokół odbioru: jak zabezpieczyć się przed roszczeniowym klientem
- [x] 2026-10-09 · Przedawnienie zachowku: od kiedy liczyć 5 lat i co przerywa bieg
- [x] 2026-10-14 · Opinia ujawniająca przebieg leczenia: co wolno pacjentowi, a czego Tobie
- [x] 2026-10-19 · Hejt w komentarzach: odpowiedzialność administratora fanpage'a i forum

## Scheduled — batch 3 (10 — co 5 dni, 24.10 – 08.12.2026)

Pierwsza partia dobrana według zasady z README (przed jaką decyzją stoi czytelnik / ile kosztuje
pomyłka / czy ktoś to tłumaczy), a nie według tieryzacji opartej na 269 wyświetleniach. Trzon
stanowią tematy transakcyjne z Tier 4, odłożone wcześniej jako „bez wyświetleń" — przy jednym
kliknięciu w trzy miesiące brak wyświetleń znaczy „nie jesteśmy widoczni", a nie „nikt nie szuka".

- [x] 2026-10-24 · Zadatek a zaliczka — czym się różnią i co przepada *(1840 sł.)*
- [x] 2026-10-29 · Zwrot zadatku — kiedy sprzedający musi oddać pieniądze *(1747 sł.)*
- [x] 2026-11-03 · Koszty notarialne przy zakupie mieszkania — taksa i PCC *(1138 sł.)*
- [x] 2026-11-08 · Podatek od sprzedaży mieszkania przed upływem 5 lat *(905 sł.)*
- [x] 2026-11-13 · Sprzedaż odziedziczonego mieszkania — formalności i podatki *(969 sł.)*
- [x] 2026-11-18 · Kupno mieszkania z lokatorem — jak wycenić ryzyko *(1072 sł.)*
- [x] 2026-11-23 · Ukryte wady mieszkania po zakupie — czego możesz żądać *(1009 sł.)*
- [x] 2026-11-28 · Zniesienie współwłasności mieszkania — ile kosztuje i ile trwa *(988 sł.)*
- [x] 2026-12-03 · Sprzedaż mieszkania z kredytem hipotecznym — co z hipoteką *(968 sł.)*
- [x] 2026-12-08 · Odpowiedzialność członka zarządu za długi spółki z o.o. *(1071 sł.)*

**Kolejka sięga 2026-12-08** — następną partię przygotować na przełom listopada i grudnia.
Wtedy będzie już nowy eksport z Search Console (ok. 2026-11-20), więc batch 4 można oprzeć
na danych, a nie na samej zasadzie doboru.

**Temat wykreślony:** „Umowa przedwstępna notarialna czy pisemna" — kanibalizowałby
`umowa-przedwstepna-pulapki`, który ma na to całą sekcję. W jego miejsce weszła sprzedaż
mieszkania z kredytem.


## Scheduled — batch 1 (10 — every 5 days, Jul 16 – Aug 30, 2026)

- [x] 2026-07-16 · Umowa najmu okazjonalnego: jak wynająć mieszkanie i się zabezpieczyć
- [x] 2026-07-21 · Eksmisja lokatora, który nie płaci: ile trwa i jak przebiega
- [x] 2026-07-26 · Niezapłacona faktura: wezwanie do zapłaty i pozew w EPU
- [x] 2026-07-31 · Darowizna mieszkania czy testament: co wybrać i co z zachowkiem
- [x] 2026-08-05 · Ktoś podszywa się pod Ciebie w internecie
- [x] 2026-08-10 · Umowa o dzieło, zlecenie czy B2B: różnice, składki i ryzyka
- [x] 2026-08-15 · Zasiedzenie nieruchomości: kiedy i jak można nabyć własność
- [x] 2026-08-20 · Negatywna opinia od byłego pracownika
- [x] 2026-08-25 · Zwrot towaru kupionego przez internet: 14 dni, wyjątki i spory
- [x] 2026-08-30 · Sprostowanie prasowe i usunięcie artykułu

---

# TIER 1 — pogłębienie tego, co już generuje wyświetlenia

Największy zwrot na włożoną pracę: mamy dowód popytu, brakuje głębokości. Wszystko 1200+ słów.
Robić przed pisaniem czegokolwiek nowego.

## 1a. Przepisanie istniejących artykułów (nie nowe treści)

**Zrobione — cały Tier 1a zamknięty 2026-09-09.** Liczby słów podane wcześniej w tej sekcji
były nieaktualne (zachowek przepisano już w a45c4a3, a backlog nadal pokazywał 602 słowa);
przy kolejnej rewizji sprawdzać stan w plikach, nie w tej liście.

- [x] **Zachowek** → 1515 sł. Tytuł na „komu **się należy**", bezpośrednia odpowiedź pod H1,
      substrat zachowku, FAQ pod długi ogon *(a45c4a3)*
- [x] **Nieuczciwy kontrahent B2B** → 1548 sł. Wchłania „windykacja b2b"; dochodzi biała lista
      VAT, art. 777 k.p.c., rekompensata 40/70/100 euro. Poprawiona wada merytoryczna:
      kara umowna za opóźnienie w **zapłacie** jest nieważna (art. 483 § 1 k.c.)
- [x] **Eksmisja lokatora** → 1349 sł. Warianty z zapytań: bez tytułu prawnego, mieszkanie
      własnościowe kupione z lokatorem, komunalne, członek rodziny bez umowy, wyłączenie
      ochrony; nazewnictwo „najem socjalny lokalu" (od 2019)
- [x] **Testament** → 1385 sł. Zapis zwykły vs windykacyjny (ten drugi tylko notarialnie),
      taksa notarialna 50/150/200 zł, testamenty szczególne i ich 6 miesięcy, przesłanki
      nieważności z terminami 3/10 lat


## 1b. Satelity klastra zachowkowego

Popyt widoczny w Search Console, brak pokrycia:

- [x] ✅ zaplanowany 2026-09-19 · Wydziedziczenie a zachowek — kiedy pozbawienie zachowku jest skuteczne (art. 1008 k.c.)
- [x] ✅ zaplanowany 2026-09-29 · Zachowek od darowizny — gdy spadkodawca rozdał majątek za życia
- [x] ✅ zaplanowany 2026-10-09 · Przedawnienie zachowku — 5 lat, od kiedy liczyć i co przerywa bieg

# TIER 2 — Reputacja (jedyna kategoria, w której realnie rankujemy)

Niska konkurencja, niski wolumen, ale top 7–15. Buduje autorytet tematyczny i zasila
usługę Reputacja (Milestone 2 — jedyna kategoria z realną intencją zakupową).
750–1000 słów wystarcza.

- [x] ✅ zaplanowany 2026-10-19 · Hejt w komentarzach — odpowiedzialność administratora fanpage'a i forum
- [ ] Stalking i uporczywe nękanie (art. 190a § 1 KK) — jak udokumentować i zgłosić
- [ ] Czarny PR konkurencji — nieuczciwa konkurencja w praktyce (art. 14 uznk)
- [ ] Krytyka firmy na YouTube i TikToku — recenzja czy naruszenie
- [ ] Deepfake i AI a wizerunek — co robić, gdy "Ty" mówisz cudze słowa
- [ ] Wygrany proces o dobra osobiste — jak wyegzekwować przeprosiny
- [ ] Wizerunek pracownika w materiałach firmy — zgoda i jej cofnięcie

Rozszerzenie niszy medycznej (tam mamy poz. 7,7 i 15,2 — najmocniejszy sygnał na stronie):
- [ ] Znieważenie lekarza w internecie — ścieżka karna czy cywilna
- [ ] Reputacja gabinetu po błędzie medycznym — komunikacja i granice prawne

## 2a. Klaster: trudny klient i asymetria tajemnicy zawodowej ★ priorytet w Tier 2

Najmocniejszy niewykorzystany temat na stronie. Łączy trzy rzeczy naraz: niszę, w której
realnie rankujemy (medyczna, poz. 7,7), zawody zaufania publicznego jako grupę docelową
retainera Reputacja, oraz problem, o którym prawie nikt nie pisze od strony prawnej.

**Oś tematyczna:** klient/pacjent może napisać publicznie wszystko, a Ty — związany tajemnicą
zawodową — nie możesz sprostować nawet oczywistego kłamstwa. Nie możesz napisać „ta osoba
nigdy nie była moim pacjentem", bo samo to jest ujawnieniem danych. Ta asymetria jest realnym
problemem lekarzy, dentystów, adwokatów, radców, psychologów i notariuszy — i nie ma na nią
dobrej treści w polskim internecie.

⚠️ **Uwaga na kanibalizację:** sam scenariusz „oddaj pieniądze albo 1★" jest już pokryty przez
`szantaz-reputacyjny-jak-reagowac`. Ścieżki usuwania opinii pokrywają `usuniecie-opinii-google`,
`rodo-a-opinie-w-internecie` i `opinia-od-pacjenta-ktorego-nie-leczyles`. Poniższe artykuły
muszą linkować do nich zamiast powtarzać, a same trzymać się swojego kąta.

**Flagowy (1200+ słów, fraza konkurencyjna):**
- [x] ✅ zaplanowany 2026-09-04 · **Tajemnica zawodowa a odpowiedź na negatywną opinię — dlaczego nie możesz się bronić
      publicznie.** Rdzeń klastra. Dlaczego odpowiedź „przyjmowałem Panią 12 marca, zabieg
      przebiegł prawidłowo" to naruszenie tajemnicy lekarskiej *i* RODO jednocześnie.
      Podstawy: art. 40 ustawy o zawodach lekarza i lekarza dentysty, art. 6 Prawa o adwokaturze,
      art. 3 ustawy o radcach prawnych, art. 9 RODO (dane o zdrowiu). Kary UODO za odpowiedzi
      z danymi pacjenta. Odpowiedzialność dyscyplinarna przed izbą — realniejsze ryzyko niż grzywna.
      Kluczowe pytanie do rozstrzygnięcia: czy opublikowanie opinii przez pacjenta zwalnia
      z tajemnicy? (Nie zwalnia — i to jest teza artykułu.)

**Satelity (750–1000 słów):**
- [x] ✅ zaplanowany 2026-09-09 · Jak odpowiedzieć na negatywną opinię, nie łamiąc tajemnicy zawodowej — konkretne szablony
      odpowiedzi, które nie potwierdzają ani nie zaprzeczają, że ktoś był klientem. Najbardziej
      praktyczny artykuł klastra i naturalny lead magnet do wzorów pism.
- [x] ✅ zaplanowany 2026-09-14 · Klient żąda zwrotu pieniędzy po wykonanej usłudze — kiedy naprawdę musisz zwrócić.
      Strona kontraktowa, nie reputacyjna: umowa o dzieło (rezultat) vs zlecenie (staranne
      działanie), rękojmia przy usługach, granice praw konsumenta, brak prawa odstąpienia
      po wykonaniu usługi za wyraźną zgodą. Odpowiada na pytanie, którego pozostałe artykuły
      nie tykają: *czy to żądanie jest w ogóle zasadne?*
- [x] ✅ zaplanowany 2026-09-24 · Groźba negatywnej opinii jako wymuszenie — kiedy to przestępstwo (art. 191 § 1 i art. 286 KK).
      Pogłębienie prawnokarne sekcji „Czy to jest przestępstwo" z artykułu o szantażu:
      granica między dozwolonym naciskiem a groźbą bezprawną, jak zabezpieczyć dowody,
      jak wygląda zawiadomienie. **Musi linkować do `szantaz-reputacyjny`, nie powtarzać go.**
- [x] ✅ zaplanowany 2026-10-04 · Umowa i protokół odbioru jako ochrona przed roszczeniowym klientem — co wpisać, zanim
      pojawi się spór. Akceptacja etapów, protokół odbioru, procedura reklamacyjna, klauzula
      o zakresie usługi. Prewencja zamiast gaszenia pożaru; łączy Reputację z Biznesem.
- [x] ✅ zaplanowany 2026-10-14 · Opinia ujawniająca przebieg leczenia lub sprawy — co wolno pacjentowi, a czego Tobie.
      Czy pacjent może opisać publicznie swoją dokumentację (może — to jego dane) i dlaczego
      nie odblokowuje to Twojego prawa do odpowiedzi.

# TIER 3 — Biznes

Umiarkowany sygnał popytu (`kontrahent b2b`, `windykacja b2b`, `b2b vs umowa zlecenie`).
Pisać po Tier 1–2.

- [ ] Kary umowne — jak zastrzegać, żeby dały się egzekwować
- [ ] Umowa NDA — co musi zawierać, żeby nie była tylko ozdobą
- [ ] Rękojmia w B2B — czy i jak można ją wyłączyć
- [ ] Wypowiedzenie umowy B2B "z dnia na dzień" — ważność krótkich terminów
- [x] Odpowiedzialność członka zarządu za długi spółki *(batch 3)* (art. 299 KSH)
- [ ] Spółka 50/50 — paraliż decyzyjny i jak z niego wyjść
- [ ] Kontrola ZUS lub US w firmie — prawa przedsiębiorcy
- [ ] Upadłość konsumencka — dla kogo, co daje i co zabiera
- [ ] Sprzedaż udziałów w sp. z o.o. — krok po kroku
- [ ] Przekształcenie JDG w sp. z o.o. — kiedy warto i ile kosztuje
- [ ] Praca zdalna — obowiązki pracodawcy i ekwiwalenty

# TIER 4 — Nieruchomości transakcyjne (odłożone)

Zero wyświetleń w 3 miesiącach na którykolwiek z tych tematów. Nie znaczy, że złe — znaczy,
że nie mamy dowodu, a przy 1200 słowach na artykuł koszt pomyłki jest realny.
**Odblokować, gdy:** pojawią się wyświetlenia na frazy transakcyjne, albo gdy Tier 1–2
będzie wyczerpany.

Najbliżej istniejącego popytu (`umowa-przedwstepna` już rankuje na poz. 7,0 — te trzy podpiąć pod nią):
- [x] Zadatek a zaliczka *(batch 3)* — różnica, którą poznaje się po fakcie
- [x] Umowa przedwstępna u notariusza czy zwykła pisemna *(batch 3)* — różnica w skutkach
- [x] Koszty notarialne przy zakupie mieszkania *(batch 3)* — taksa, PCC, wpisy

Powiązane z dziedziczeniem (naturalne linkowanie do klastra spadkowego):
- [x] Sprzedaż odziedziczonego mieszkania *(batch 3)* — formalności, podatki, zgody spadkobierców
- [ ] Umowa dożywocia — jak działa i czym różni się od darowizny
- [x] Zniesienie współwłasności przed sprzedażą *(batch 3)* — sposoby, koszty, podatki

Reszta (kolejność bez zmian, priorytet niski):
- [ ] Umowa rezerwacyjna z deweloperem — co naprawdę podpisujesz
- [ ] Rachunek powierniczy dewelopera — czy Twoje pieniądze są bezpieczne
- [ ] Kupno działki budowlanej — MPZP, warunki zabudowy, dostęp do drogi i media
- [ ] Kupno mieszkania na licytacji komorniczej — okazja czy pułapka
- [x] Kupno mieszkania z lokatorem *(batch 3)* — na co uważać i jak wycenić ryzyko
- [x] Wady ukryte po zakupie *(batch 3)* — rękojmia na rynku wtórnym krok po kroku
- [ ] Cesja umowy deweloperskiej — jak bezpiecznie kupić (i sprzedać) cesję
- [x] Sprzedaż mieszkania przed upływem 5 lat *(batch 3)* — podatek 19% i jak go legalnie uniknąć
- [ ] Ulga mieszkaniowa w PIT — "własne cele mieszkaniowe" w praktyce urzędów
- [x] Sprzedaż mieszkania z kredytem hipotecznym *(batch 3)* — promesa, spłata przez notariusza
- [ ] Pełnomocnictwo do sprzedaży nieruchomości — zakres, forma i ryzyka nadużycia
- [ ] Umowa z pośrednikiem — klauzula wyłączności i prowizja po wygaśnięciu
- [ ] Samowola budowlana a sprzedaż domu — legalizacja przed transakcją
- [ ] Podział majątku po rozwodzie a sprzedaż mieszkania na wspólnym kredycie

---

## Milestones

- [ ] **Wydzielić kategorię Dziedziczenie/Spadki.** Mamy już 7 artykułów spadkowych
      (testament, odrzucenie spadku, stwierdzenie nabycia, dział spadku, podatek od spadku,
      zachowek, darowizna vs testament) schowanych pod Nieruchomości przez tagi w
      `src/utils/categories.ts`. To największy klaster tematyczny na stronie i nie ma własnej
      strony-huba ani spójnego linkowania wewnętrznego.
      ⚠️ **Tylko dodatkowo + przekierowania** — `/nieruchomosci/` musi dalej działać
      (zasada: nigdy nie zmieniamy URL-i bez redirectu).
- [ ] Strona usługi Reputacja z formularzem zgłoszenia (Milestone 2)
- [ ] **Narzędzia jako magnes na linki (slot 10% z miksu treści w README)** — brakujący
      odpowiednik trackera cen z ogrzeje.pl. Kandydaci, w kolejności siły:
      - [ ] Kalkulator kosztów zakupu mieszkania — taksa notarialna + PCC 2% + wpisy do KW.
            Kwota, którą każdy kupujący i tak musi policzyć, a nikt nie podaje jej łącznie
      - [ ] Kalkulator zachowku — udział ustawowy, stawka ½ lub ⅔, doliczane darowizny
      - [ ] Zestawienie opłat sądowych i notarialnych, aktualizowane przy zmianie rozporządzeń
- [ ] Interlinking pass: każdy nowy artykuł linkuje do 2–3 istniejących (i odwrotnie przy okazji edycji)
- [ ] Poprosić o reindeksację kanonicznych URL-i (zachowek, opinia-od-pacjenta) — stare wersje
      `http://` i bez ukośnika wciąż zbierają wyświetlenia zamiast kanonicznych
- [ ] Ponowny eksport z Search Console za 3 miesiące (ok. 2026-11-20) i re-tieryzacja powyższego
